CREATE OR REPLACE PACKAGE BODY TBCIS.iCALCULATE_DISCOUNTS AS
   /*
   ...
   */
   ...
   BEGIN
      ...
      FOR rec_start IN c_start (l_sudi_start) LOOP
         ...
         IF l_ref_num_c IS NULL THEN   --soodustuse komponenti pole registreeritud, registreeri!
            ...
            --
            icalculate_discounts.insert_sudi (l_sudi, p_success, p_error_text);
         ...
         IF NVL (l_cadc.bill_sel_sum, 'Y') = 'Y' THEN
            ...
            icalculate_discounts.find_amount_dica_fcit (p_susg_ref_num
         ...
         ELSE   --hhcc
            ...
            icalculate_discounts.find_amount_dica (p_susg_ref_num
                                                 ,l_ref_num_c
                                                 ,l_bill_period
                                                 ,l_in_month
                                                 ,l_prev_period
         ...
         IF NVL (l_cadc.DURATION, 0) > l_full_duration OR l_cadc.DURATION IS NULL THEN
            ...
            ELSE
               ...
               IF p_sepv_ref_num IS NULL THEN
                  IF p_sept_type_code IS NOT NULL THEN
                     l_days_disc := icalculate_fixed_charges.find_pack_days (rec_start.sudi_start
                                                                           ,l_end
                                                                           ,p_susg_ref_num
                                                                           ,0
                                                                           );
                  ELSE
                     l_days_disc := icalculate_fixed_charges.find_num_of_days (rec_start.sudi_start
                                                                             ,l_end
                                                                             ,p_susg_ref_num
                                                                             ,p_sety_ref_num
                                                                             ,0
               ...
               ELSE   -- p_sepv_ref_num IS NULL
                  icalculate_fixed_charges.get_num_of_days (rec_start.sudi_start
                                                          ,l_end
                                                          ,p_susg_ref_num
                                                          ,p_sety_ref_num
                                                          ,p_sepa_ref_num
            ...
            icalculate_discounts.calculate_one_amount (l_duration   --kestus
            ...
            IF l_disc_am_ref_num IS NULL THEN   --soodustust pole enne seda arvutatud(uus dica)
               ...
               icalculate_discounts.insert_dica (p_dica, p_success, p_error_text);
   ...
   BEGIN
      ...
      FOR rec_y IN c_all_cadc (l_dico_ref_num) LOOP
         ...
         IF l_ref_num IS NULL THEN   --soodustust pole registreeritud (sisestan)
            ...
            l_sudi.end_date     := iCalculate_Discounts.calculate_sudi_end_date (l_sudi.dico_ref_num, l_sudi.start_date);  -- DOBAS-1315
            --
     ...
     ,p_once_off        IN  VARCHAR2
      ...
      CURSOR c_disc_type IS
         SELECT discount_type
          ...
          WHERE pro_rata = p_pro_rata AND regular_charge = p_regular_charge AND once_off = p_once_off;
                        ...
                        IF NVL (l_parent_dico_rec.mobile_part, 'A') = 'A' THEN
                           ...
                           IF     NVL (l_parent_dico_rec.month_of_serv, NVL (l_month_of_serv, 0)) <=
                              ...
                              IF l_parent_ok THEN
                                 ...
                                 l_parent_sudi.end_date     := iCalculate_Discounts.calculate_sudi_end_date (l_parent_sudi.dico_ref_num, l_parent_sudi.start_date); --DOBAS-1315
                                 --
   ...
   PROCEDURE validate_manual_discount (
     ...
     ,p_sept_type_code  IN      serv_package_types.type_code%TYPE DEFAULT NULL
      ...
      l_vmct_maac_type              statements.vmct_maac_type%TYPE;
   BEGIN
      l_found := icalculate_discounts.get_part_maac_data (p_maac_ref_num
                                                        ,l_chca_type_code
                                                        ,l_bicy_cycle_code
                                                        ,l_month_of_serv
                                                        ,l_party_type
      ...
      IF l_found THEN
         ...
         --
         p_success := icalculate_discounts.chk_apply_discount (p_dico_rec
                                                             ,p_susg_ref_num
                                                             ,p_maac_ref_num
                                                             ,p_deal_ref_num
                                                             ,p_region

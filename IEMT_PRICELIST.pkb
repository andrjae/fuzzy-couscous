CREATE OR REPLACE PACKAGE BODY TBCIS.iemt_pricelist IS
   ...
   BEGIN
      ...
      FETCH c
      INTO p_pro_rata, p_once_off, p_regular_charge, p_daily_charge;
   ...
   ------------------------
   PROCEDURE ins_ebpl (p_ebpl        IN     iemt_bill_price_list%ROWTYPE
      ...
      --
      l_key_sety_ref_num            iemt_bill_price_list.key_sety_ref_num%TYPE;
      l_key_charge_value            iemt_bill_price_list.key_charge_value%TYPE;
   BEGIN
      --
      OPEN c_key;
      ...
      --
      INSERT
        INTO iemt_bill_price_list (fcty_type
                                 ,what
                                 ,list_type
                                 ,fcit_type_code
                                 ,taty_type_code
   ...
   BEGIN
      ...
      VALUES (p_bame_row.module_ref
             ...
             ,NVL (p_bame_row.run_by_user, 'TBCIS')
   ...
   PROCEDURE ebpl_maac_services (p_start_date  IN     DATE -- period start
      ...
      --
      l_what                        iemt_bill_price_list.what%TYPE := 'Konto teenused';
      l_list_type_p                 iemt_bill_price_list.list_type%TYPE := 'Põhihind';
      l_list_type_e                 iemt_bill_price_list.list_type%TYPE := 'Erihind';
      ...
      l_daily_charge                fixed_charge_item_types.daily_charge%TYPE;
      l_count                       INTEGER := 0;
      ...
      CURSOR c_sety_gr (
        ...
        ,p_daily_charge  IN fixed_charge_item_types.daily_charge%TYPE) IS
      ...
      CURSOR c_prli (
        ...
        ,p_once_off        IN fixed_charge_item_types.once_off%TYPE
        ,p_regular_charge  IN fixed_charge_item_types.regular_charge%TYPE
        ,p_daily_charge    IN fixed_charge_item_types.daily_charge%TYPE
   ...
   BEGIN -- Start
      ---
      --- 2. võta lipud:
      iemt_pricelist.get_flags (l_pro_rata, l_once_off, l_regular_charge, l_daily_charge, p_fcty_type);

      -- 1. kustuta eelmine seis:
      DELETE iemt_bill_price_list
      ...
      FOR i IN c_sety_gr (l_pro_rata, l_once_off, l_regular_charge, l_daily_charge) LOOP
         l_curr_start := NULL;
         ...
         FOR j IN c_chca LOOP
            ...
            FOR k IN c_prli (l_pro_rata, l_once_off, l_regular_charge, l_daily_charge, i.sety, i.sepa, i.sepv) LOOP
               ...
               --3.FICV LOOP
               FOR l
                  IN c_ficv (l_pro_rata, l_once_off, l_regular_charge, l_daily_charge, i.sety, i.sepa, i.sepv, j.chca, l_curr_start) LOOP
                  ...
                  IF GREATEST (k.start_date, l_curr_start) < l.start_date THEN
                     ...
                     iemt_pricelist.ins_ebpl (l_ebpl, l_success, l_error_text);
      ...
      --
      UPDATE iemt_bill_price_list a1
         SET chargeable = 'N'
       WHERE what = l_what AND fcty_type = p_fcty_type
             AND sety_ref_num IN
                    (SELECT   sety_ref_num
                         FROM iemt_bill_price_list a
                        WHERE a.what = l_what AND a.fcty_type = p_fcty_type
                              AND NOT EXISTS
                                         (SELECT 1
                                            FROM iemt_bill_price_list b
   ...
   PROCEDURE mob_sety_charges (p_fcty_type         fixed_charge_types.type_code%TYPE
      ...
      l_ebpl_row                    iemt_bill_price_list%ROWTYPE;
      ...
      l_daily_charge                fixed_charge_item_types.daily_charge%TYPE := 'N';
      ...
      PROCEDURE mob_prli_sety_charges (p_success           IN OUT BOOLEAN
                                      ...
                                      ,p_daily_charge             fixed_charge_item_types.daily_charge%TYPE
                                      ) IS
         ...
         BEGIN
            FOR rec_pp
               ...
               IF p_last_date = p_end_date THEN
                  ...
                  iemt_pricelist.ins_ebpl (l_ebpl_row, l_success, p_error_text);
      ...
      BEGIN
         ...
         FOR rec_p
            ...
            IF p_last_date = p_end_date THEN
               ...
               iemt_pricelist.ins_ebpl (l_ebpl_row, p_success, p_error_text);
            ELSE
   ...
   BEGIN
      ...
      FOR r_pack IN c_pack (l_start_date) LOOP
         ...
         FOR c_serv IN c_service (l_pro_rata, l_once_off, l_regular_charge, l_daily_charge) LOOP
   ...
   PROCEDURE start_ebpl (p_fcty_type IN fixed_charge_types.type_code%TYPE DEFAULT 'MCH') IS
      ...
      --
      l_what                        iemt_bill_price_list.what%TYPE := NULL;
      l_list_type_p                 iemt_bill_price_list.list_type%TYPE := 'Põhihind';
      l_list_type_e                 iemt_bill_price_list.list_type%TYPE := 'Erihind';
      --------------------------------------------------------------------------------

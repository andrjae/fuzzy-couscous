...
/******************************************************************************
   ...
   ---------  ----------  ---------------  ------------------------------------
   1.0        2018.11.21      AndresJaek       1. Created this package body.
******************************************************************************/
  ...
  PROCEDURE proc_daily_charges (  
      ...
      c_proc_module_ref    CONSTANT VARCHAR2 (10) := 'BCCU1545';
  ...
  BEGIN
      gen_bill.msg (c_proc_module_ref
                   ,c_module_name
                   ,c_message_nr
...
--**--**--**--**
      ...
      IF NOT l_success THEN
         ...
         --
         gen_bill.msg (c_proc_module_ref, c_module_name, c_message_nr, l_message, NULL, p_bill_cycle);
      ELSE
         l_result := 1;
   ...
   EXCEPTION
      ...
      WHEN OTHERS THEN
         ROLLBACK;
         gen_bill.msg (c_proc_module_ref, c_module_name, c_message_nr, SQLERRM, NULL, p_bill_cycle);
         gen_bill.msg (c_proc_module_ref
                      ,c_module_name
                      ,c_message_nr
                      , 'Protsess lõpetanud veaga ' || TO_CHAR (SYSDATE, 'dd.mm.yyyy hh24:mi:ss') || '. '
  ...
  END;
   ...
   BEGIN
        ...
        greatest(t.starts, nvl(mipo_start_date, :1)) starts, least(t.ends, nvl(mipo_end_date, :2)) ends, 
        t.sepv_start_real, t.sety_ref_num, t.prli_charge_value, t.ficv_charge_value, t.charge_value, t.curr_code, t.sept_type_code, t.sept_description, t.sepv_ref_num, t.price_type,
        ...
        q'[qq4.*,   
        -least(sum(qq4.discount) over (partition by qq4.susg order by qq4.rn) - qq4.discount + qq4.charge_value, - qq4.discount) discountx, 
        ...
        int_inv.invo_end, sesu.serv_num,
        row_number() over (partition by qq4.susg, qq4.starts, qq4.sudi_ref_num order by sesu.end_date desc nulls first) r1
        from qq4 LEFT JOIN int_inv on int_inv.susg_ref_num = qq4.susg AND int_inv.billing_selector = qq4.fcit_billing_selector and int_inv.fcit_type_code = qq4.fcit_type_code
                 LEFT JOIN senu_susg sesu on qq4.susg = sesu.susg_ref_num
        ...
        qq6.packet_code packet_discount, qq6.starts, qq6.ends, qq6.main_days, qq6.charge_value, qq6.price_type, qq6.curr_code, qq6.period_charge, qq6.incl_disc, qq6.fcit_billing_selector, qq6.fcit_type_code, 
        qq6.fcit_taty_type_code, qq6.fcit_bise, qq6.fcit_desc, qq6.real_disc_start disc_start, qq6.real_disc_end disc_end, qq6.disc_days, 
        qq6.discountx discount, qq6.discount_code, qq6.discount_description,
...
PROCEDURE fill_inen_temp(p_start_date DATE) 
   ...
   insert into daily_charges_inen_temp
    ...
    null VMCT_RATE_VALUE, disc_days NUM_OF_DAYS, null EVRE_DATA_VOLUME, null MAAS_REF_NUM, cadc_ref_num CADC_REF_NUM, fcit_fcdt_type_code FCDT_TYPE_CODE, null SEC_FF_DISC_AMT, null SEC_CURR_CODE, 
    ...
    case when nvl(q4.invo_amount,0)=0 then q4.eek_amt when q4.invo_amount > 0 then greatest(q4.eek_amt, 0) else least(q4.eek_amt, 0) end EEK_AMT, 
    ...
    where acc_amount != 0 or eek_amt != 0
    ;
   END;   
   ...
   insert into daily_charges_inen_s_temp
    ...
    EVRE_CHAR_USAGE, PRINT_REQUIRED, VMCT_RATE_VALUE, SUM(NUM_OF_DAYS) num_of_days, EVRE_DATA_VOLUME, MAAS_REF_NUM, max(CADC_REF_NUM) cadc_ref_num, FCDT_TYPE_CODE, SEC_FF_DISC_AMT, SEC_CURR_CODE, 
    round(sum(EEK_AMT),2) eek_amt, AMT_TAX, PRI_CURR_CODE, FF_DISC_AMT, FIXED_CHARGE_VALUE, SEC_ACC_AMOUNT, round(SUM(ACC_AMOUNT),4) acc_amount, BILLING_SELECTOR_TEXT, ENTRY_TEXT, 
    ADDITIONAL_ENTRY_TEXT, maac, sudi_ref_num
    from (
    select /*+ NO_PARALLEL */null REF_NUM, null INVO_REF_NUM, null SEC_AMT, 'N' ROUNDING_INDICATOR, 'N' UNDER_DISPUTE, null CREATED_BY, sysdate DATE_CREATED, null AMT_IN_CURR, disc_billing_selector BILLING_SELECTOR, 
    null FCIT_TYPE_CODE, fcit_taty_type_code TATY_TYPE_CODE, susg SUSG_REF_NUM, null IADN_REF_NUM, null CURR_CODE, null VMCT_TYPE_CODE, null LAST_UPDATED_BY, null DATE_UPDATED, null DESCRIPTION, 
    null SEC_AMT_TAX, null AMT_TAX_CURR, 'N' MANUAL_ENTRY, 1 EVRE_COUNT, null EVRE_DURATION, '???' MODULE_REF, null SEC_FIXED_CHARGE_VALUE, null EVRE_CHAR_USAGE, crm PRINT_REQUIRED, 
    ...
    CADC_REF_NUM, FCDT_TYPE_CODE, SEC_FF_DISC_AMT, SEC_CURR_CODE, EEK_AMT - nvl(invo_amount, 0) eek_amt, AMT_TAX, PRI_CURR_CODE, FF_DISC_AMT, FIXED_CHARGE_VALUE, 
    ...
    row_number() over (partition by inen.billing_selector, inen.fcit_type_code, inen.susg_ref_num order by null) rn, 
    q1.*    
    ...
    from (
    select /*+ no_index (INVO invo_pest_i1) */ 
    sum(case when invo.ref_num is null then 0 else coalesce(inen.eek_amt, inen.acc_amount,0) end) over (partition by inen.billing_selector, inen.fcit_type_code, inen.susg_ref_num, q2.sudi_ref_num)  invo_amount,
    row_number() over (partition by inen.billing_selector, inen.fcit_type_code, inen.susg_ref_num, q2.sudi_ref_num order by null) rn, 
    q2.*    
    from q2 LEFT JOIN invoice_entries inen on q2.billing_selector = inen.billing_selector and q2.susg_ref_num = inen.susg_ref_num and q2.fcdt_type_code = inen.fcdt_type_code
                                               and q2.cadc_ref_num = inen.cadc_ref_num
            LEFT JOIN invoices invo on inen.invo_ref_num = invo.ref_num and invo.invoice_type = 'INT' and invo.period_start = p_start_date
    )
    where rn = 1
    )

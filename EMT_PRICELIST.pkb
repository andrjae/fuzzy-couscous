...
CREATE OR REPLACE PACKAGE BODY emt_pricelist IS
   ...
   BEGIN
      ...
      FETCH c
      INTO p_pro_rata, p_once_off, p_regular_charge;
   ...
   PROCEDURE ebpl_maac_services (p_start_date  IN     DATE -- period start
      ...
      l_regular_charge              fixed_charge_item_types.pro_rata%TYPE;
      l_count                       INTEGER := 0;
      ...
      CURSOR c_sety_gr (
        ...
        ,p_regular_charge  IN fixed_charge_item_types.regular_charge%TYPE) IS
   ...
   BEGIN -- Start
      ---
      --- 2. võta lipud:
      emt_pricelist.get_flags (l_pro_rata, l_once_off, l_regular_charge, p_fcty_type);
      ...
      FOR i IN c_sety_gr (l_pro_rata, l_once_off, l_regular_charge) LOOP
         l_curr_start := NULL;
         ...
         FOR j IN c_chca LOOP
            ...
            FOR k IN c_prli (l_pro_rata, l_once_off, l_regular_charge, i.sety, i.sepa, i.sepv) LOOP
               ...
               --3.FICV LOOP
               FOR l
                  IN c_ficv (l_pro_rata, l_once_off, l_regular_charge, i.sety, i.sepa, i.sepv, j.chca, l_curr_start) LOOP
   ...
   BEGIN
      ...
      FOR r_pack IN c_pack (l_start_date) LOOP
         ...
         FOR c_serv IN c_service (l_pro_rata, l_once_off, l_regular_charge) LOOP
